...
function DesktopConversationList() {
  ...
  // Compute template availabilities
  const templateMatches = useMemo(
    () =>
      conversationTemplates.map((tmpl) => {
        const matched = matchTemplateModels(tmpl, enabledModels);
        return {
          template: tmpl,
          matched,
          missingCount: tmpl.participants.length - matched.length,
          canCreate: matched.length >= 2,
        };
      }),
    [enabledModels],
  );
  ...
  const handleTemplateSelect = useCallback(
    async (template: (typeof conversationTemplates)[number]) => {
      const matched = matchTemplateModels(template, enabledModels);
      if (matched.length < 2) return; // Need at least 2 participants for a group

      const members: SelectedMember[] = matched.map((m) => ({
        modelId: m.matchedId,
        identityId: m.identityId,
      }));

      const conv = await createConversation(members[0].modelId, undefined, members);
      // Set the group system prompt
      await updateGroupSystemPrompt(conv.id, template.groupSystemPrompt);
      setCurrentConversation(conv.id);
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <Dialog open={showTemplatePicker} onOpenChange={(v) => !v && setShowTemplatePicker(false)}>
        <DialogContent className="max-h-[70vh] max-w-md">
          <DialogHeader>
            <DialogTitle className="text-base">从模板创建群组</DialogTitle>
            <DialogDescription className="text-xs">
              选择一个预设模板，快速创建多模型群聊
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-3 overflow-y-auto px-1 pb-2">
            {templateMatches.map(({ template, matched, missingCount, canCreate }) => {
              const icons: Record<string, React.ReactNode> = {
                "research-debate": <FlaskConical size={18} className="text-purple-500" />,
                "writing-polish": <FileText size={18} className="text-blue-500" />,
                "code-review": <Code size={18} className="text-green-500" />,
                brainstorm: <Lightbulb size={18} className="text-amber-500" />,
              };
              return (
                <button
                  key={template.id}
                  ...
                  className={`flex items-start gap-3 rounded-xl border p-4 text-left transition-all ${
                    canCreate
                      ? "hover:bg-accent cursor-pointer border-border hover:border-primary/30"
                      : "cursor-not-allowed opacity-50 border-dashed border-border"
                  }`}
                >
                  <div className="mt-0.5 flex-shrink-0">
                    {icons[template.id] ?? <Users size={18} className="text-muted-foreground" />}
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1">
                      {template.participants.map((p) => {
                        const isMatched = matched.some((m) => m.modelId === p.modelId);
                        return (
                          <span
                            key={p.modelId}
                      ...
                      {missingCount > 0 && (
                        <span className="text-muted-foreground inline-flex items-center rounded-full px-2 py-0.5 text-[10px]">
                          缺 {missingCount} 个模型
                        </span>
                      )}
                    </div>
                    {canCreate && (
                      <p className="text-primary mt-2 text-xs font-medium">点击创建 →</p>
                    )}
                    {!canCreate && (
