...
import { ensureDeviceRegistered } from "./services/device-service";
...
export default function App() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // Auto-configure 映创云 supplier on first launch
      const providers = useProviderStore.getState().providers;
      const hasYingchuang = providers.some(
        (p) => p.baseUrl && p.baseUrl.includes("hwzs.club"),
      );
      if (!hasYingchuang) {
        const newProvider = {
          id: generateId(),
        ...
        // Auto-fetch models so the user can start chatting immediately
        useProviderStore.getState().fetchModels(newProvider.id).catch(() => {
